...
export default function DevicesPage() {
  ...
  // Interface viewer states
  const [isInterfacesModalOpen, setIsInterfacesModalOpen] = useState(false);
  ...
  const handleViewInterfaces = async (device) => {
    setViewingDevice(device);
    setIsInterfacesModalOpen(true);
    setLoadingInterfaces(true);
    setInterfacesError(null);
    setDeviceInterfaces([]);
    try {
      const res = await ndsApi.getDeviceInterfaces(device.id);
      setDeviceInterfaces(res.data?.data || res.data || res || []);
    } catch (err) {
      console.error(err);
      setInterfacesError('ไม่สามารถดึงข้อมูลอินเตอร์เฟสของอุปกรณ์ได้');
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-base-950/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-4xl rounded-xl border border-base-600 bg-base-900 p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-left flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center border-b border-base-600/30 pb-3">
              ...
              <button 
                onClick={() => setIsInterfacesModalOpen(false)}
                className="text-ink-400 hover:text-ink-100 transition-colors"
              >
                ✕
              </button>
            ...
            <div className="flex-1 overflow-y-auto mt-4 pr-1">
              {loadingInterfaces ? (
                <div className="py-12 text-center text-xs text-ink-400 font-mono animate-pulse">
                  ⚙️ กำลังโหลดข้อมูล Interfaces จาก NetBox...
                </div>
              ) : interfacesError ? (
                ...
                <div className="py-12 text-center text-xs text-ink-500 font-mono">
                  ไม่พบข้อมูลอินเตอร์เฟสสำหรับดีไวซ์นี้
                </div>
              ) : (
                <div className="overflow-x-auto rounded-xl border border-base-600/60 bg-base-950/40">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className="bg-base-900/80 text-xs font-mono uppercase text-ink-400 border-b border-base-600/50">
                      <tr>
                    ...
                    <tbody className="divide-y divide-base-600/30">
                      {deviceInterfaces.map((iface) => (
                        <tr key={iface.id} className="hover:bg-base-800/30 transition-colors">
                          <td className="px-5 py-3.5 font-mono text-xs font-semibold text-ink-100">{iface.name}</td>
                            ...
                            {(() => {
                              ...
                              return (
                                <select
                                  ...
                                  className={`rounded px-2 py-0.5 text-[10px] font-semibold font-mono uppercase focus:outline-none focus:ring-1 focus:ring-nds focus:border-nds cursor-pointer transition-all border ${selectColor}`}
                                >
            ...
            <div className="mt-6 flex justify-end border-t border-base-600/30 pt-4">
              <button
                type="button"
                onClick={() => setIsInterfacesModalOpen(false)}
                className="rounded-lg border border-base-600 bg-base-950 px-5 py-2 text-xs text-ink-400 hover:bg-base-800 transition-colors font-semibold"
              >
                ปิดหน้าต่าง
              </button>
            </div>
